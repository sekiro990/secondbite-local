...
router.post('/:id/claim', verifyToken, async (req, res) => {
    try {
        ...
        // Create a notification iont he table after claiming a food

        const userRes = await pool.query('SELECT name FROM users WHERE id = $1', [user_id]);
        const claimerName = userRes.rows[0]?.name || 'Someone';

        await pool.query(
            `INSERT INTO notifications (user_id, message)
             VALUES ($1, $2)`,
            [Listingresult.rows[0].created_by, `${claimerName} claimed your food item!`]
        );
    } catch (err) {
